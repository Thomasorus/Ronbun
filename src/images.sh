...
function resize () {
  ...
  do
    ...
    # create smaller sizes for responsive image selection
      ...
      if [[ ! -f $output ]]; then
        ...
        # if [[ $width -ge $size ]]; then
          ...
          convert $file -resize $size -filter Triangle -define filter:support=2 -unsharp 0.25x0.08+8.3+0.045 -quality 82 -define jpeg:fancy-upsampling=off $output

          convert $file -resize $size -filter Triangle -define filter:support=2 -unsharp 0.25x0.08+8.3+0.045 -quality 82 -define webp:lossless=true -define webp:auto-filter=true $outputwebp
    ...
    if [[ ! -f $output ]]; then
      echo -n "| ${MAXWIDTH} | ${file}"
      convert $file -resize $MAXWIDTH -filter Triangle -define filter:support=2 -unsharp 0.25x0.08+8.3+0.045 -quality 82 -define jpeg:fancy-upsampling=off $output

      convert $file -resize $MAXWIDTH -filter Triangle -define filter:support=2 -unsharp 0.25x0.08+8.3+0.045 -quality 82 -define webp:lossless=true -define webp:auto-filter=true $outputwebp
