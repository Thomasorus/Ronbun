...
async function generateHtml (
  ...
  for (let i = 0; i < contentArray.length; i++) {
    ...
    if (
      el.name.toLowerCase() !== el.host.toLowerCase() &&
      el.host !== undefined
    ) {
    ...
    if (error === 'ENOENT') {
      ...
      const titleContent = RegExp(titlePattern).exec(
        existingPage
      )
      const htmlTitle =
        titleContent[1].replace(/\n/g, '').trim() ===
        el.title.replace(/\n/g, '').trim()

      const brefPattern = '<meta name="description" content="([\\s\\S]*?)">'
      const brefContent = RegExp(brefPattern).exec(existingPage)
      const htmlBref =
        brefContent[1].replace(/\n/g, '').trim() ===
        el.bref.replace(/\n/g, '').trim()

      const textPattern = '<article>([\\s\\S]*?)<\\/article>'
      const textContent = RegExp(textPattern).exec(existingPage)
      let htmlText
      if (textContent) {
        if (el.name !== 'Time') {
          htmlText =
            textContent[1].replace(/\n/g, '').trim() ===
            el.html.replace(/\n/g, '').trim()
        } else {
          htmlText = false
        }
      ...
      let htmlHostSlug
      const hostSlugPattern = 'html">([\\s\\S]*?)<\\/a><\\/i><\\/nav>'
      const HostSlugContent = RegExp(hostSlugPattern).exec(existingPage)

      if (HostSlugContent !== null) {
        htmlHostSlug =
          HostSlugContent[1].replace(/\n/g, '').trim() ===
          el.host.replace(/\n/g, '').trim()
      } else {
        htmlHostSlug = true
      }

      const contentPattern = 'content update: <time>([\\s\\S]*?)<\\/time>'
      const contentChange = RegExp(contentPattern).exec(existingPage)
...
async function generateAssets (buildAssetsDir, srcAssetsDir) {
  ...
  const logoChange = await utils.copyAsset(
    buildAssetsDir,
    srcAssetsDir,
    'logo.png'
  )
  const templateChange = await utils.copyAsset(
    buildAssetsDir,
    srcAssetsDir,
    'main.html'
  )
  const styleChange = await utils.copyAsset(
    buildAssetsDir,
    srcAssetsDir,
    'style.css'
  )
...
async function generateRss (
  ...
  for (let i = 0; i < contentArray.length; i++) {
    ...
    if (item.priv === 'false') {
      ...
      const datePattern = 'content update: <time>([\\s\\S]*?)<\\/time>'
      const pageDate = RegExp(datePattern).exec(page)
...
async function generateAll () {
  ...
  const styleHasChanged = await generateAssets(
    config.buildAssetsDir,
    config.srcAssetsDir
  )
  ...
  const textContentArray = await utils.splitContent(
    textContent,
    config.contentSplitter
  )
