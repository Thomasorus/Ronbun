...
function generateHtml(arr) {
    Object.entries(arr).forEach(([key, value]) => {
        ...
        if (el.children.length > 0 && el.menu === "true") {
            let children = `<nav aria-label="Subpages"><dl>`
            if(el.slug === "journal") {
                el.children.reverse()
            }
            el.children.map(x => {
                ...
                if(x.priv === "false") {
                    children += `<dt><a href="${x.slug}.html" >${x.name}</a></dt><dd>${x.bref}</dd>`
                }
            })
            children += "</dl></nav>"
            el.submenu = children
        ...
        page = page.replace(/subMenu/g, el.submenu ? el.submenu : "")
