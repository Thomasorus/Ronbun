...
//         } else {
//             const destSize = fs
//                 .statSync(`${destinationDir}/${file.name}`)
//                 .size.toString()
//                 .trim();
//             const sourceSize = fs
//                 .statSync(`${sourceDir}/${file.name}`)
//                 .size.toString()
//                 .trim();
...
//         const gitDate = await execSync(
//             `cd ${folder} && git log -1 --format="%ci" './${file}'`
//         )
//             .toString()
//             .trim();
...
module.exports = {
    setMainCategories: function setMainCategories(arr, category) {
...
//                   <span class="time-row__title">Statistics for: ${
//                       el.name
...
//             el.hor += `<line x1="0" y1="${i * chartScale + chartScale}" x2="${
//                 el.scaleX
//             }" y2="${
//                 i * chartScale + chartScale
//             }" stroke="var(--text, #000)" opacity="0.3" stroke-width="0.5px" />`;
//             el.numbers += `<text fill="var(--text, #000)" x="0" y="${
//                 el.scaleY - 1 - i * chartScale
//             }">${i}</text>`;
//         }
//         el.numbers += "</svg></g>";
//         el.svg = `
//           <div aria-hidden="true" style="overflow-x: auto;">
//               <p style="text-align:center; max-width: ${
//                   el.scaleX
//               }px; margin-left:${el.scaleY / chartScale}px;">${el.year}</p>
//               <div style="font-size:12px; text-align:center; width:10px; display:inline-block; transform:rotate(-90deg) translateX(${
//                   (el.scaleY - chartScale) / 2
//               }px);">hours</div>
//                   <svg viewbox="0 0 ${el.scaleX} ${
//             el.scaleY + chartScale
//         }" width="${el.scaleX}" height="${
//             el.scaleY + chartScale
//         }" style="border:2px solid var(--text, #000);">
//                   ${el.hor}
//                   <path d="${
//                       el.path
//                   }" fill="none" stroke="var(--a, #000)" stroke-width="2px"/>
//                   ${el.dots}
//                   ${el.numbers}
//               </svg>
//               <div style="width:${
//                   el.scaleX
//               }px; text-align:center; font-size:12px; line-height: 1.1; margin-left:${
//             el.scaleY / chartScale
//         }px;">weeks</div>
//               </div>
//           </div>
...
// export function checkDate(page, feed) {
//     const feedText = readFile(feed).split("</item>\n<item>");
//     let date;

//     feedText.forEach((el) => {
//         let titleP = `<title>(${page.name}) - Thomasorus<\/title>`;
//         let re = new RegExp(titleP, "");
//         let rssTest = re.test(el);
//         let rssTitle = re.exec(el);
//         if (rssTest) {
//             let elbody = el
//                 .split("<![CDATA[")
//                 .pop()
...
//                 .replace(/(\s|\r\n|\r|\n)/g, "")
//                 .trim();
//             let pagebody = page.body.replace(/(\s|\r\n|\r|\n)/g, "").trim();
//             const contentTest = elbody === pagebody ? true : false;
//             if (contentTest) {
...
//                 }
//             } else {
//                 console.log("new content new date");
